...
public class ContinentTest {
	...
	/**
	 *This method tests if the addcountry method actually adds the country into the list or not.
	 */
	@Test
	public void checkAddCountry() {
		cont1.addCountry(c1);
		listOfCountries = cont1.getCountryList();
		countryNameInList = listOfCountries.stream().filter(x->x.getCountryName()==countryName).map(x -> x.getCountryName()).collect(Collectors.joining());
