...
public class GameTest {
	...
	public void testGamePlayRandomTimes()
	{
		...
		while(iterationCount>0)
		{
			...
			ArrayList<Country> playerCountries = game.getPlayerCountries();
			List<Integer> countryIds = playerCountries.stream()
			...
			reinforcementCount = reinforcementCount < 3 ? 3
					: reinforcementCount;
			...
			ArrayList<Country> neigbouringCountries = game.getNeighbouringCountriesForFortification(fromCountry.getCountryId());
			...
			if(neigbouringCountries!= null && neigbouringCountries.size() > 0)
			{
				...
				if(neigbouringCountries.size() == 1)
				{
					...
					toCountry = neigbouringCountries.get(Common.getRandomNumberInRange(0, neigbouringCountries.size() -1));
				}
				...
				if(previousFromCountryArmiesCount > 1)
				{
					...
					boolean isFortifySuccessFull = game.fortifyCountry(fromCountry.getCountryId(), toCountry.getCountryId(), armyCountToMove);
